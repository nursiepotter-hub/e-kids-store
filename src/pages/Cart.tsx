      ...
      <div className="min-h-screen pt-28 pb-20 flex items-center justify-center">
        <div className="text-center max-w-md mx-auto px-4">
          <div className="w-20 h-20 mx-auto mb-6 rounded-2xl bg-cream flex items-center justify-center border-2 border-charcoal/5">
            <ShoppingBag className="w-10 h-10 text-red" />
          </div>
          <h2 className="font-display text-3xl text-charcoal mb-3">Votre panier est vide</h2>
          <p className="text-sage font-bold mb-8">Découvrez notre sélection de jouets éducatifs.</p>
          <Link
            to="/produits"
            className="inline-flex items-center gap-2 bg-red hover:bg-red-light text-white px-8 py-4 rounded-full font-bold text-lg transition-all shadow-lg shadow-red/30"
          >
            Voir les jouets
          </Link>
        </div>
  ...
  return (
    <div className="min-h-screen pt-28 pb-20">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        <Link to="/produits" className="inline-flex items-center gap-2 text-sage hover:text-red font-bold transition-colors mb-8">
        ...
        <div className="grid lg:grid-cols-3 gap-10">
            ...
            {items.map((item) => (
              <div key={item.id} className="bg-white rounded-[1.5rem] p-4 sm:p-6 flex gap-4 border-2 border-charcoal/5 shadow-sm">
                ...
                <div className="flex-1 min-w-0">
                  <span className="text-xs text-blue font-bold uppercase tracking-wider">{item.category}</span>
                  ...
                  <div className="flex items-center gap-4 mt-3">
                    <div className="flex items-center border-2 border-charcoal/10 rounded-full overflow-hidden">
                      <button
                        ...
                        className="w-8 h-8 flex items-center justify-center text-charcoal/60 hover:text-red hover:bg-cream transition-colors font-bold"
                      >
                        −
                      </button>
                    ...
                    <button
                      ...
                      className="text-sage hover:text-red transition-colors"
                    >
                ...
                <div className="text-right shrink-0">
                  <span className="font-display text-lg font-bold text-red">{(item.price * item.quantity).toLocaleString()} F</span>
                </div>
              </div>
            ))}
          </div>
          ...
          <div>
            <div className="bg-white rounded-[1.5rem] p-6 sm:p-8 border-2 border-charcoal/5 shadow-sm sticky top-28">
              ...
              <Link
                to="/checkout"
                className="block w-full bg-red hover:bg-red-light text-white text-center py-4 rounded-full font-bold text-lg transition-all shadow-lg shadow-red/30"
              >
                Commander
              </Link>
