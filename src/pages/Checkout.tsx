    ...
    return (
      <div className="min-h-screen pt-28 pb-20 flex items-center justify-center">
        <div className="text-center max-w-md mx-auto px-4">
          <div className="w-20 h-20 mx-auto mb-6 rounded-2xl bg-mint/20 flex items-center justify-center">
          ...
          <button
            onClick={() => navigate('/')}
            className="bg-coral hover:bg-coral-light text-white px-8 py-4 rounded-full font-bold transition-all shadow-lg shadow-coral/30"
          >
            Retour à l'accueil
          </button>
        </div>
  ...
  return (
    <div className="min-h-screen pt-28 pb-20">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        <button onClick={() => navigate('/panier')} className="flex items-center gap-2 text-sage hover:text-coral font-bold transition-colors mb-8">
        ...
        <div className="grid lg:grid-cols-5 gap-10">
          <form onSubmit={handleSubmit} className="lg:col-span-3">
            <span className="text-teal font-bold tracking-widest text-sm uppercase">Finaliser</span>
              ...
              <div className="grid sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <input name="nom" value={form.nom} onChange={handleChange} required
                    className="w-full px-4 py-3.5 rounded-full border-2 border-charcoal/10 focus:border-coral focus:ring-2 focus:ring-coral/20 outline-none bg-cream/50 font-semibold" />
                </div>
                <div>
                    ...
                    <button
                      ...
                      className={`flex flex-col items-center gap-2 p-4 rounded-2xl border-2 transition-all ${
                        payment === m.value
                          ...
                          : 'border-charcoal/10 hover:border-coral/50'
                      }`}
                    >
            ...
            <button
              ...
              className="w-full bg-coral hover:bg-coral-light disabled:opacity-50 text-white py-4 rounded-full font-bold text-lg transition-all mt-6 shadow-lg shadow-coral/30 flex items-center justify-center gap-2"
            >
                ...
                {items.map((item) => (
                  <div key={item.id} className="flex gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-sage font-semibold">x{item.quantity}</p>
                    </div>
                    <span className="text-sm font-bold text-coral whitespace-nowrap">{(item.price * item.quantity).toLocaleString()} F</span>
                  </div>
                ))}
              </div>
